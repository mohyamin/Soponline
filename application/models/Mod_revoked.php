...
class Mod_revoked extends CI_Model
{
    var $table = 'v_tbl_revoked'; // Menggunakan View sebagai tabel
    var $column_search = array('id_file', 'nama_file', 'tahun', 'created_at', 'nama_revoked');
    var $column_order = array('id_file', 'nama_file', 'tahun', 'created_at', 'nama_revoked');
    ...
    public function insert_revoked($data, $lampiran_files)
    {
        // Simpan data utama
        $this->db->insert('tbl_revoked', $data);
        ...
        foreach ($lampiran_paths as $path) {
            $this->db->insert('tbl_revoked_lampiran', [
                'id_file' => $id_file,
                'file_url' => $path,
    ...
    {
        $this->db->insert('tbl_revoked_lampiran', $data); // Menggunakan tabel lampiran
    }

    // Fungsi update data compliance
    public function update_revoked($id, $data)
    {
        $this->db->where('id_file', $id);
        $this->db->update('tbl_revoked', $data);
    ...
    {
        $this->db->where('id_file', $id);
        return $this->db->get('tbl_revoked')->row(); // Mengambil data dari tabel asli
    }
    ...
    {
        $this->db->where('id_file', $id);
        $this->db->delete('tbl_revoked');
    }
    // Fungsi untuk mendapatkan URL file berdasarkan ID
    function getImage($id)
    {
        $this->db->select('url_file');
        $this->db->from('tbl_revoked');
        $this->db->where('id_file', $id);
        return $this->db->get();
    }
    public function getLampiran($id_file)
    {
        return $this->db->get_where('tbl_revoked_lampiran', ['id_file' => $id_file]);
    }
    public function getComplianceWithLampiran($id)
    {
        $compliance = $this->db->get_where('tbl_revoked', ['id_file' => $id])->row_array();
        $lampiran = $this->db->get_where('tbl_revoked_lampiran', ['id_file' => $id])->result_array();
