...
function slug($s)
{
	// Pastikan $s adalah string, jika tidak set default ke string kosong
	if (is_null($s)) {
		$s = '';
	}

	// Pastikan $s adalah string
	if (!is_string($s)) {
		$s = (string) $s;
	}
	...
	// Hapus karakter yang ada di array $d
	$s = str_replace($d, '', $s);

	// Ganti spasi dengan tanda - dan ubah huruf menjadi kecil
