...
class Viewer extends MY_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('Mod_viewer'));
    }
    ...
    public function index()
    {
        ...
        $this->template->load('layoutbackend', 'admin/viewer', $data);
    }
    ...
    public function ajax_list()
    {
        ...
        set_time_limit(3600);
        $list = $this->Mod_viewer->get_datatables();
        $data = array();
        $no = 1;
        foreach ($list as $pel) {
            $row = array();
            ...
            if ($this->session->userdata('id_level') == "9") {
                ...
                $row[] = '<a target="_blank" href="' . base_url('Viewer/previewPDF/') . $pel->id_file . '" title="' . htmlspecialchars($pel->nama_file, ENT_QUOTES, 'UTF-8') . '"><i class="fas fa-eye"></i></a>';
            } else {
            ...
            $lampiranList = $this->db->get_where('tbl_compliance_lampiran', ['id_file' => $pel->id_file])->result();

            $lampiranHtml = '';
            foreach ($lampiranList as $lamp) {
                // Buat URL dasar lampiran
                $fileUrl = base_url('uploads/lampiran/' . $lamp->nama_file);
                $previewUrl = base_url('Viewer/previewPDF/' . $lamp->nama_file);
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Mod_viewer->count_all(),
            "recordsFiltered" => $this->Mod_viewer->count_filtered(),
            "data" => $data,
        );
    ...
    public function previewPDF($id)
    {
        ...
        if (is_numeric($id)) {
            ...
            $data['detail'] = $this->db->get_where('tbl_compliance', ['id_file' => $id])->row();
        } else {
            // Ini untuk lampiran
