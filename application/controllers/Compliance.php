...
class Compliance extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->load('layoutbackend', 'admin/compliance', $data);
    }
        ...
        if (!empty($_FILES['lampiran']['name'][0])) {
            ...
            for ($i = 0; $i < $lampiranCount; $i++) {
                ...
                if ($this->upload->do_upload('file_lampiran')) {
                    ...
                    $existing = $this->db->get_where('tbl_compliance_lampiran', [
                        'id_file' => $id_file,
                    ...
                    if ($existing == 0) {
                        ...
                        $this->db->insert('tbl_compliance_lampiran', $lampiran);
                    }
                }
            }
        }
    ...
    public function update()
    {
        ...
        // Ambil data lama
        $oldData = $this->mod_compliance->getImage($id)->row_array();
        ...
        $this->db->update('tbl_compliance', $updateData);
    ...
    {
        $data = $this->mod_compliance->getComplianceWithLampiran($id);
    ...
    {
        $compliance = $this->Mod_compliance->get_compliance_by_id($id_file); // Ambil data compliance
    ...
    {
        // Cari lampiran di DB
        $lampiran = $this->db->get_where('tbl_compliance_lampiran', ['id_lampiran' => $id_lampiran])->row();
        ...
        if ($lampiran) {
            ...
            $this->db->delete('tbl_compliance_lampiran');

            // Catat log user
            $savelog = [
                'id_user'    => $this->session->userdata('id_user'),
                'fitur'      => "Menu Compliance - Lampiran",
    ...
    {
        // Ambil data lampiran
        $lampiran = $this->db->get_where('tbl_compliance_lampiran', [
        ...
        if ($lampiran) {
            ...
            $this->db->delete('tbl_compliance_lampiran', ['id_lampiran' => $id_lampiran]);
