...
class InternalViewer extends MY_Controller
{

    function __construct()
    {
        parent::__construct();
        $this->load->model(array('Mod_InternalViewer'));
    }
    ...
    public function index()
    {
        ...
        $this->template->load('layoutbackend', 'admin/internalviewer', $data);
    }
    ...
    public function ajax_list()
    {
        ...
        $list = $this->Mod_InternalViewer->get_datatables();
        $data = array();
        $no = 1;
        foreach ($list as $pel) {
            $row = array();
            ...
            // Cek level user dan tentukan akses
            if ($this->session->userdata('id_level') == "9") {
                // Role viewer hanya dapat melihat (preview)
                $row[] = '<a target="_blank" href="' . base_url('InternalViewer/previewPDF/') . $pel->id_file . '" title="' . htmlspecialchars($pel->nama_file, ENT_QUOTES, 'UTF-8') . '"><i class="fas fa-eye"></i></a>';
            } else {
            ...
            $lampiranList = $this->db->get_where('tbl_internal_lampiran', ['id_file' => $pel->id_file])->result();

            $lampiranHtml = '';
            foreach ($lampiranList as $lamp) {
                // Buat URL dasar lampiran
                $fileUrl = base_url('uploads/lampiran/' . $lamp->nama_file);
                $previewUrl = base_url('InternalViewer/previewPDF/' . $lamp->nama_file);
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Mod_InternalViewer->count_all(),
            "recordsFiltered" => $this->Mod_InternalViewer->count_filtered(),
            "data" => $data,
        );
    ...
    public function insert()
    {
        ...
        if ($this->upload->do_upload('file')) {
            ...
            // $this->Mod_InternalViewer->insert("tbl_internal", $save);
    ...
    public function update()
    {
        ...
        $this->Mod_InternalViewer->update_kat($id, $save);
        echo json_encode(array("status" => TRUE));
    }

    public function edit_kat($id)
    {
        $data = $this->Mod_InternalViewer->get_kat($id);
    ...
    {
        $id = $this->input->post('id');
        $this->Mod_InternalViewer->delete_kat($id, 'kategori_internal');
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function previewPDF($id)
    {
        ...
        if (is_numeric($id)) {
            ...
            $data['detail'] = $this->db->get_where('tbl_internal', ['id_file' => $id])->row();
        } else {
            // Ini untuk lampiran
            // Bangun data manual karena tidak pakai tabel tbl_internal
            $data['detail'] = (object)[
                'url_file' => base_url('uploads/lampiran/' . $id)
            ];
        }

        $this->load->view('admin/internalviewerpdf', $data);
    }
}
