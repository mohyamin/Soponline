...
class Revoked extends MY_Controller
{

    function __construct()
    {
        parent::__construct();
        $this->load->model(array('mod_revoked'));
    }
    ...
    public function index()
    {
        ...
        $this->template->load('layoutbackend', 'admin/revoked', $data);
    }
    ...
    public function ajax_list()
    {
        ...
        set_time_limit(3600);
        $list = $this->mod_revoked->get_datatables();
        $data = array();
        $no = 1;
        foreach ($list as $pel) {
            $row = array();
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->mod_revoked->count_all(),
            "recordsFiltered" => $this->mod_revoked->count_filtered(),
            "data" => $data,
        );
    ...
    public function insert()
    {
        ...
        $file = $this->_upload_file('file', 'uploads', 'pdf|docx|png', 100000, '1363', '2000');

        if ($file) {
            $save = array(
                'nama_file'  => $this->input->post('nama_file'),
                'id_kateg'     => $this->input->post('id_kateg'),
            ...
            $this->db->insert('tbl_revoked', $save);
        ...
        if (!empty($_FILES['lampiran']['name'][0])) {
            ...
            for ($i = 0; $i < $lampiranCount; $i++) {
                ...
                if ($this->upload->do_upload('file_lampiran')) {
                    ...
                    $existing = $this->db->get_where('tbl_revoked_lampiran', [
                        'id_file' => $id_file,
                    ...
                    if ($existing == 0) {
                        ...
                        $this->db->insert('tbl_revoked_lampiran', $lampiran);
                    }
                }
            }
        }
    ...
    public function update()
    {
        ...
        // Ambil data lama
        $oldData = $this->mod_revoked->getImage($id)->row_array();
        ...
        $this->db->update('tbl_revoked', $updateData);
    ...
    {
        $data = $this->mod_revoked->getComplianceWithLampiran($id);
    ...
    {
        $id = $this->input->post('id_file');
        $data = $this->mod_revoked->delete_kat($id, 'tbl_revoked');
        $savelog = array(
            'id_user'      => $this->session->userdata('id_user'),
            'fitur'        => "Menu Revoked",
    ...
    {
        $compliance = $this->mod_revoked->get_compliance_by_id($id_file); // Ambil data compliance
    ...
    {
        // Cari lampiran di DB
        $lampiran = $this->db->get_where('tbl_revoked_lampiran', ['id_lampiran_rev' => $id_lampiran])->row();
        ...
        if ($lampiran) {
            ...
            // Hapus dari DB
            $this->db->where('id_lampiran_rev', $id_lampiran);
            $this->db->delete('tbl_revoked_lampiran');

            // Catat log user
            $savelog = [
                'id_user'    => $this->session->userdata('id_user'),
                'fitur'      => "Menu Revoked - Lampiran",
    ...
    {
        // Ambil data lampiran
        $lampiran = $this->db->get_where('tbl_revoked_lampiran', [
            'id_lampiran_rev' => $id_lampiran
        ])->row();
        ...
        if ($lampiran) {
            ...
            $this->db->delete('tbl_revoked_lampiran', ['id_lampiran_rev' => $id_lampiran]);
    ...
    private function _validate()
    {
        ...
        if ($this->input->post('id_kateg') == '') {
            $data['inputerror'][] = 'id_kateg';
